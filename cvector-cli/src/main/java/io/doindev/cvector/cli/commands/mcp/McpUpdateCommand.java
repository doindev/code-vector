...
public class McpUpdateCommand implements Callable<Integer> {
    ...
    public Integer call() throws Exception {
        ...
        CvectorConfig updated = svc.update(root, before -> {
            ...
            // Preserve any existing timeouts + session-recovery config the user set via
            // settings.json or the REST API — `cvector mcp update` only changes url + transport.
            CvectorConfig.McpConfig next = new CvectorConfig.McpConfig(
                    newUrl, canonical, current.timeouts(), current.localhostSessionRecovery());
