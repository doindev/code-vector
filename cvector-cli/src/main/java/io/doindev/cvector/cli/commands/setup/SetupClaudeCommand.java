...
public class SetupClaudeCommand implements Callable<Integer> {
    ...
    @Option(names = "--config", description = "Override the Claude Desktop config path.")
    private Path configPath;

    private final CvectorRuntime runtime;

    public SetupClaudeCommand(CvectorRuntime runtime) {
        this.runtime = runtime;
    }

    @Override
    public Integer call() throws Exception {
        Path cfg = configPath != null ? configPath : IdeSetupUtil.claudeDesktopConfigPath();
        ...
        System.out.println("registered cvector MCP server in Claude Desktop config:");
        System.out.println("  config:  " + cfg);
        System.out.println("  cwd:     " + projectRoot);
        System.out.println("  jar:     " + jar);
        System.out.println("Restart Claude Desktop to load the new server.");
        return 0;
    }
}
