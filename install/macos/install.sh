#!/usr/bin/env bash
# install.sh -- build + install cvector on macOS.
#
...
PIDS="$(pgrep -f 'cvector\.app/Contents/MacOS/cvector|cvector\.jar|io\.doindev\.cvector\.CvectorApplication' 2>/dev/null | grep -v "^$$\$" || true)"
if [[ -n "${PIDS}" ]]; then
    echo ""
    echo "cvector is running (PIDs: $(echo ${PIDS} | tr '\n' ' '))"
    if ask_yes_no "Stop them before installing?" y; then
        # SIGTERM first, grace period, then SIGKILL.
        for pid in ${PIDS}; do kill -TERM "${pid}" 2>/dev/null || true; done
        for _ in 1 2 3 4 5; do
            sleep 1
            still="$(pgrep -f 'cvector\.app/Contents/MacOS/cvector|cvector\.jar|io\.doindev\.cvector\.CvectorApplication' 2>/dev/null | grep -v "^$$\$" || true)"
            [[ -z "${still}" ]] && break
        done
        if [[ -n "${still:-}" ]]; then
...
if [[ ! -d "${APP_BUNDLE}" ]]; then
    ...
    echo "(if you're on Linux, run install/linux/install.sh instead)" >&2
    exit 1
fi

# 3. Wipe any prior install and copy the fresh bundle.
...
ln -sf "${INSTALL_DIR}/Contents/MacOS/cvector" "${BIN_LINK}"
echo "linked ${BIN_LINK} -> ${INSTALL_DIR}/Contents/MacOS/cvector"
